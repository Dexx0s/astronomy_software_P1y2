...
#funcion para validar el tipo de dato en el entry
def validar_numero(P):
    entrada_coord_z.insert(1, imagen_actual + 1) #si saco esto el entry se vuelve loco pero no deja usar el teclado "un alma por otra al parecer"
    if P.isdigit():
        return True
    else:
        return False
...
def actualizar_etiqueta_coordenadas(event=None):
...
##entry
validacion_numeros = ventana.register(validar_numero)
entrada_coord_z = tk.Entry(ventana, validate="key", validatecommand=(validacion_numeros, "%P"))
entrada_coord_z.grid(row=2, column=4, padx=5, pady=5)

# Asociar la función "cargar_imagen" al evento <Return> del Entry
entrada_coord_z.bind("<Return>", cargar_imagen)
